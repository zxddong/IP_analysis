...
public class IPSeeker {
	...
	* 用来封装ip相关信息，目前只有两个字段，ip所在的国家和地区
	...
	static {
		...
		IP_FILE = filepath.substring(filepath.lastIndexOf(":") + 1, filepath.length());
		logger.info("IP_FILE={}", IP_FILE);
	 ...
	 * 
	 * @author
	 * @param ipFilePath ip地址库.dat文件的路径
	 * @return
	 */
	...
	private IPSeeker() {
		...
		try {
			...
			logger.info("/qqwry.dat ={}", IPSeeker.class.getResource("/qqwry.dat").toString());
			logger.info("IP_FILE ={}", IP_FILE);
			logger.info("IP地址信息文件没有找到，IP显示功能将无法使用");
			ipFile = null;
		...
		if (ipFile != null) {
			try {
				...
				logger.info("IP地址信息文件格式有错误，IP显示功能将无法使用 ={}", e);
				ipFile = null;
			}
		}
	}
		...
		for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
			...
			if (temp != -1) {
				...
				if (loc.country.indexOf(s) != -1 || loc.area.indexOf(s) != -1) {
					...
					// 得到起始IP
					readIP(offset - 4, b4);
					entry.beginIp = IPTransform.getIpStringFromBytes(b4);
					// 得到结束IP
					readIP(temp, b4);
					entry.endIp = IPTransform.getIpStringFromBytes(b4);
					// 添加该记录
					ret.add(entry);
				}
			}
	...
	public String getCountry(byte[] ip) {
		...
		// 保存ip，转换ip字节数组为字符串形式
		String ipStr = IPTransform.getIpStringFromBytes(ip);
	...
	public String getCountry(String ip) {
		return getCountry(IPTransform.getIpByteArrayFromString(ip));
	}
	...
	public String getArea(String ip) {
		return getArea(IPTransform.getIpByteArrayFromString(ip));
	}
	...
	private long locateIP(byte[] ip) {
		...
		for (long i = ipBegin, j = ipEnd; i < j;) {
			...
			readIP(m, b4);
			r = compareIP(ip, b4);
			// log.debug(IPTransform.getIpStringFromBytes(b));
			if (r > 0) {
                i = m;
            }else if (r < 0) {
				if (m == j) {
	...
	private String readString(long offset) {
		try {
			...
			if (i != 0) {
                return IPTransform.getString(buf, 0, i, "GBK");
            }
		} catch (IOException e) {
			logger.info(e.getMessage());
		}
	...
	public String getAddress(String ip) {
		String country = StringUtils.equalsIgnoreCase(getCountry(ip).trim(), "CZ88.NET") ? "" : getCountry(ip);
		String area = StringUtils.equalsIgnoreCase(getArea(ip).trim(), ("CZ88.NET")) ? "" : getArea(ip);
